...
#define MYDIR "\Users\nikol\OneDrive\Рабочий стол\MIPT\Информатика\ct"

char newfiles[256] = "\Users\nikol\OneDrive\Рабочий стол\MIPT\Информатика\ct\backups";
...
void dif(const char * file, const char * name){
	...
	int fd2 = open(newfile, O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRGRP|S_IROTH);
        int fd = open("Users\nikol\OneDrive\Рабочий стол\MIPT\Информатика\ct\backups\log.txt", O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRGRP|S_IROTH);
        if(fd < 0) {
                exit(1);
        }
	dup2(fd, 1);
	...
	system("rm Users\nikol\OneDrive\Рабочий стол\MIPT\Информатика\ct\backups\log.txt");
	close(fd);
	close(fd2);
	return;
}
...
int iftext(const char * file){
	...
	dup2(fd, 1);
        system(arg);
	close(fd);
        fd = open("Users\nikol\OneDrive\Рабочий стол\MIPT\Информатика\ct\backups\log.txt",O_RDONLY);
        struct stat statbuf;
        fstat(fd, &statbuf);
